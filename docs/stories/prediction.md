...
Scope: Student submission of predictions on both BINARY and CONTINUOUS questions, immutability enforcement, ownership, and the lock-time boundary contract defined in ForecastQuestion Story 4.
A scope note before the stories: your original Excel-based design (from the About_YFL materials) also tracked an investment amount, an optional line of credit for leverage, a forecasting method, a rationale ("why"), and time spent, all feeding into an ROI calculation alongside accuracy. None of that appears in your actual Roadmap, Backend Checklist, or your v1 scoring decision (binary correctness + confidence multiplier). I'm treating the investment/ROI/leverage mechanic as out of scope for this component since it's not in your working docs, rather than quietly building toward it. If you want that mechanic in the beta too, it changes Prediction meaningfully (needs a funds ledger, debt tracking, and a third scoring dimension) and is worth its own explicit decision before I write stories around it. Flag it if so.
...
ADR entry: investment/ROI/leverage mechanic explicitly deferred out of v1 scope, noted so it doesn't get silently reintroduced by a future contributor referencing the original marketing materials.
