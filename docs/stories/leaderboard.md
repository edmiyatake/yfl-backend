...
Scope: Read/aggregation layer over Scoring's output. No new writes of its own beyond what's needed for caching; this component queries and ranks data that Scoring already produced.
One computation choice worth stating before the stories: for "Most Improved," comparing a trailing window against a baseline that includes the same predictions would be circular, the recent window would be judged against itself. I'm defining baseline as the student's average accuracy across all resolved predictions before the trailing window, and recent as the average of the last N predictions (window size configurable at the season level, defaulting to 10, following the same pattern as the financial config fields added to Season). A student needs at least window + 1 resolved predictions to qualify for a Most Improved ranking at all, otherwise there's no real "before" period to compare against, they just don't appear on that specific leaderboard yet, which is a legitimate state, not an error.
...
As a user, I want to see students ranked by portfolio return, so that I know who's making the best financial decisions, not just the most accurate calls.
Acceptance Criteria:
...
As a user, I want to see teams ranked by accuracy and by ROI, so that team competition is visible alongside individual standing.
Acceptance Criteria:
...
As a user, I want the leaderboard to reflect the latest resolved question shortly after it resolves, so that standings stay current per the brief's "everyone can see their leaderboard all the time" requirement.
Acceptance Criteria:
